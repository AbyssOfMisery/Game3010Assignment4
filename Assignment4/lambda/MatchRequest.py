...
ddb_table = dynamodb.Table('Players')

def handler(event, context):
    playerId = event['user_id']
    playerPass = event['Password']
    playerScore = -1
    
    response = {'TicketId' : 'AuthError' }
    
    result = ddb_table.get_item( Key= {'user_id' : playerId } )
 
    if 'Item' not in result:
        # Create Item
        ddb_table.put_item( Item={ 'user_id' : playerId, 'Password' : playerPass, 'Score' : 1000, 'Win' : 0, 'Lose' : 0} )
        playerScore = 1000
